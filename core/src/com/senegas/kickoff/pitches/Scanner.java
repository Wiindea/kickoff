...
public class Scanner implements FootballDimensions {
	...
	public void draw() {
		...
		match.shapeRenderer.rect(this.origin.x, this.origin.y, (float) PITCH_WIDTH_IN_PX * ratio,
				(float) PITCH_HEIGHT_IN_PX * ratio);
		...
		match.shapeRenderer.circle(
				(float) (this.origin.x + (match.getBall().getPosition().x - OUTER_TOP_EDGE_X) * ratio),
