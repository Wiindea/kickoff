...
/**
 * Any tactic must be a subclass of Tactic Areas and corners numbers are defined
 * as follow.
 * 
 * <pre>
 *             Opponent team side
...
public class Tactic {
	enum Location {
		area1("area1"), area2("area2"), area3("area3"), area4("area4"), area5("area5"), area6("area6"), area7("area7"),
		area8("area8"), area9("area9"), area10("area10"), area11("area11"), area12("area12"),
		kickoff_own("kickoff_own"), kickoff_def("kickoff_def"), goalkick_own("goalkick_own"),
		goalkick_def("goalkick_own"), corner1("corner 1"), corner2("corner 2"), corner3("corner 3"),
		corner4("corner 4");
	...
	public static final int REGIONS_COUNT = REGION_ROWS * REGION_COLUMNS;
	private static final float REGION_WIDTH_IN_PX = (float) (Pitch.PITCH_WIDTH_IN_PX / REGION_COLUMNS);
	private static final float REGION_HEIGHT_IN_PX = (float) (Pitch.PITCH_HEIGHT_IN_PX / REGION_ROWS);
	...
	public Tactic(Team team, String fileName) {
		...
		try {
			...
			LOGGER.log(Level.SEVERE, "IOException occurred: " + e.toString());
		}
	}
	...
	private void loadFromXml(String fileName) throws IOException {
		...
		for (Element player : players) {
			...
			// positions
			Array<Element> positions = player.getChildrenByName("region");
			for (Element region : positions) {
				String regionName = region.get("name"); // region name

				this.locations[playerIndex][Location.valueOf(regionName).ordinal()] = new Vector2(region.getFloat("x"),
						region.getFloat("y"));
			}
			playerIndex++;
		}
	...
	public void update(Ball ball) {
		...
		for (int playerIndex = 0; playerIndex < 10; playerIndex++) {
			Vector3 playerLocation = PitchUtils.pitchToGlobal(
					team.getDirection() == Direction.NORTH ? locations[playerIndex][regionIndex].x
							: (float) Pitch.PITCH_WIDTH_IN_PX - locations[playerIndex][regionIndex].x,
					team.getDirection() == Direction.NORTH ? locations[playerIndex][regionIndex].y
							: (float) Pitch.PITCH_HEIGHT_IN_PX - locations[playerIndex][regionIndex].y);
	...
	* My team side      WIDTH
	...
	public static int getRegionIndex(Ball ball, Team team) {
		...
		if (xCoord < 0) {
			xCoord = 0;
		} else if (xCoord >= REGION_COLUMNS) {
			xCoord = REGION_COLUMNS - 1;
		} // else: do nothing

		if (yCoord < 0) {
			yCoord = 0;
		} else if (yCoord >= REGION_ROWS) {
			yCoord = REGION_ROWS - 1;
		} // else: do nothing
	...
	/**
	 * Debug only method that displays active region and tactic's player home
	 * location
	 * 
	 * @param camera
	 * @param ball
	...
	public void showRegionAndExpectedPlayerLocation(OrthographicCamera camera, Ball ball) {
		...
		for (int playerIndex = 0; playerIndex < 10; playerIndex++) {
			...
			Vector3 playerLocation = PitchUtils.pitchToGlobal(locations[playerIndex][regionIndex].x,
					locations[playerIndex][regionIndex].y);
